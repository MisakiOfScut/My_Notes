...
在实施一级的基础上，要求事务读取数据 A 时必须加 S 锁，读取完马上释放 S 锁。

可以解决读脏数据问题，因为如果一个事务在对数据 A 进行修改，根据 1 级封锁协议，会加 X 锁，那么这时就不能再加 S 锁了，也就是不会读入数据。
...
```sql
lock-x(A)...lock-s(B)...lock-s(C)...unlock(A)...unlock(C)...unlock(B)//释放锁A后不能再获取其他锁
```
...
一个事务只能读取已经提交的事务所做的修改。换句话说，一个事务所做的修改在提交之前对其它事务是不可见的。有时候也叫做不可重复读 （nonrepeatable read），因为两次执行同样的查询，可能会得到不 一样的结果。

### 可重复读（REPEATABLE READ）

保证在同一个事务中多次读取同一数据的结果是一样的。但是理论上，可重复读 隔离级别还是无法解决另外一个幻读（Phantom Read）的问题，Indoob引擎通过加入MVCC来解决幻读的问题。
